...
# Initialize a hashtable to count changed field occurrences
$changedFieldCounts = @{}

# Populate the hashtable with changed field counts
foreach ($change in $VersionHistory) {
    $changedField = $change.'Changed Field'
    $changedFieldCounts[$changedField] = ($changedFieldCounts[$changedField] ?? 0) + 1
}

# Add count information to each change
$orderedVersionHistory = $VersionHistory | ForEach-Object {
...
# Export data to Excel with pivot table and chart
$orderedVersionHistory | Export-Excel -Path "ChangeFieldAnalysis.xlsx" -ClearSheet -IncludePivotTable -PivotRows "Changed Field" -PivotData @{ 'Changed Field Count' = 'Sum' } -IncludePivotChart -ChartType ColumnClustered

Write-Host "Analysis complete. Excel file saved as ChangeFieldAnalysis.xlsx"
